...
<jsp:include page="editConfigWin.jsp">
    <jsp:param name="version" value="${project.TEST_VERSION}"/>
</jsp:include>
  	...
  	<tbody>
    	<c:forEach items="${pagelist.result}" var="config">
    		<tr id='row-<c:out value="${config.configId}"/>'>
               	...
               	<td>
					<a data-content='<c:out value="${config.testValue}"/>' data-toggle="popover" tabindex="0" data-trigger="focus" data-placement="bottom">
                  	<script type="text/javascript">
                  		var value = '${fn:replace(fn:replace(config.testValue, newLineChar, " "), newLineChar2, " ")}';
                  		if(value.length > 30)
               	...
               	<td>
                  	<c:out value="${config.testUser}"/>
               	</td>
               	<td>
					<fmt:formatDate value="${config.testTime}" pattern="yyyy-MM-dd HH:mm:ss" />
               	</td>
               	<td>
					<a href='javascript:updateConfig(<c:out value="${config.configId}"/>)' title="更新"><i class="glyphicon glyphicon-edit"></i></a>
					<a href='javascript:history("${config.configId}","${project.TEST_VERSION}")' title="历史记录"><i class="glyphicon glyphicon-leaf"></i></a>
				</td>
          	</tr>
     	</c:forEach>
	</tbody>
