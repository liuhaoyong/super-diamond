...
public class IndexContoller extends BaseController {
	...
	public void index(ModelMap modelMap) {
		...
		List<ConfProject> projects = projectService.queryProjectForUser(user);
		for(ConfProject project : projects) {
			List<ConfRoleVo> roles = projectService.queryRoles(project.getId(), user.getId());
