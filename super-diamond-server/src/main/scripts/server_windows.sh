...
# windows开发服务启动, 使用Git Bash
# cd super-diamond-new/super-diamond-server
# mvn clean install assembly:single -Pdevelopment
# mkdir target/super-diamond
# tar -xzvf target/super-diamond-bin.tar.gz -C target
# ./target/super-diamond/bin/server_windows.sh start
# http://localhost
if [ -z "$BASE_DIR" ] ; then
  PRG="$0"
...
function start_server() {
    ...
    echo "$APP_JVM_ARGS -DBASE_HOME=$BASE_HOME -DSERVER_NAME=$SERVER_NAME-$HOST_NAME \
	...
	java $APP_JVM_ARGS -DBASE_HOME=$BASE_HOME -DSERVER_NAME=$SERVER_NAME-$HOST_NAME \
	  -Xdebug -Xrunjdwp:transport=dt_socket,server=y,address=$REMOTE_DEBUG_PORT,suspend=n \
   	  $BASE_APP_ARGS com.github.diamond.jetty.JettyServer
    echo $! > $PID_FILE
